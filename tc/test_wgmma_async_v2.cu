...
// nvcc test_wgmma_async.cu -O3 -gencode arch=compute_90a,code=sm_90a -o
// test_wgmma_async srun -n1 -p h01 --gres=gpu:1 ./test_wgmma_async
// // Use the following in fuse0
// nvcc test_wgmma_async_v2.cu -O3 -U_GNU_SOURCE -gencode
// arch=compute_90a,code=sm_90a -o test_wgmma_async_v2 srun -n1
// --gres=gpu:H100:1 ./test_wgmma_async_v2
#include <cfloat>
#include <cstdint>
#include <cstdio>
#include <cstdlib>
...
__device__ void load_repeating_pattern(int i, int j) {
  // load the following
  // 8 x 16B for no swizzle
  // 8 x 32B for 32B swizzle
  // 8 x 64B for 64B swizzle
  // 8 x 128B for 128B swizzle
}
...
// (K Major) For N: This is the stride from the first col to the second col of
// the 8x2 brick in INTERLEAVED
...
/*
However, it is confusing. Don't try to read PTX doc directly, when you have no
idea on tc coordinates. Consider the following first:
1. Assume doing A @ B^T, where A and B are both row-major, or in nv-word,
k-major, i.e. C[i][j] += A[i][k] * B[j][k];
...
6. Since wgmma is 64 x N x 32B, the core matrix grid for A must be ((64 / 8) x
(32 / 16)) = (8 x 2)
7. Core matrix grid for B is (N/8 x 2)
8. Adjacent core matrices in a column of the core matrix grid must be
contiguous.
9. Elements inside a core matrix are also contiguous.
   - Run the code below and read the coordinate mapping of A, assume you do 64 x
64 x 16 with dtype as fp16/bf16:
...
10. If you want to cascade multiple wgmma window, then you add to sA (sB) MMA_M
* MMA_K (* sizeof(T))
11. You may want to consider using TMA to load to Smem. Then you may want to use
3D TMA descriptor
12. This version does NOT consider swizzling. If you want to do so, understand
the non-swizzled version first.
*/
__global__ void gemm_wgmma_bf16_krnl(size_t m, size_t n, size_t k, float *C,
  ...
  alignas(128) __shared__ nv_bfloat16 sB[BLOCK_N * BLOCK_K];

  size_t threads_per_block = blockDim.x * blockDim.y;

  auto _sA = reinterpret_cast<int4 *>(sA);
  auto _gA = reinterpret_cast<const int4 *>(A);
  auto i_ = tid % MMA_M;
  auto k_ = tid / MMA_M;
  // ONE core matrix is 8x16B FOR no swizzle mode.
  _sA[i_ + k_ * MMA_M] = _gA[i_ * (k / CORE_MAT_COLS) + k_];

  auto _sB = reinterpret_cast<int4 *>(sB);
  auto _gB = reinterpret_cast<const int4 *>(B);
  auto j_ = tid % MMA_N;
  auto k__ = tid / MMA_N;
  _sB[j_ + k__ * MMA_N] = _gB[j_ * (k / CORE_MAT_COLS) + k__];
  __syncthreads();
  ...
  constexpr int sA_mma_off = MMA_M * MMA_K / sizeof(nv_bfloat16);
  desc_a |= ((uint16_t)__cvta_generic_to_shared(sA)) >> 4;
  desc_a |= ((/*bM*/ BLOCK_M * 16) >> 4) << 16; // LBO
  desc_a |= ((8ULL * 16) >> 4)
            << 32; // SBO Note that a core matrix must be size of 8 x 16B.
  ...
  __asm__ __volatile__(
      ...
      : "+f"(rC[0]), "+f"(rC[1]), "+f"(rC[2]), "+f"(rC[3]), "+f"(rC[4]),
        ...
        "+f"(rC[10]), "+f"(rC[11]), "+f"(rC[12]), "+f"(rC[13]), "+f"(rC[14]),
        "+f"(rC[15]), "+f"(rC[16]), "+f"(rC[17]), "+f"(rC[18]), "+f"(rC[19]),
        "+f"(rC[20]), "+f"(rC[21]), "+f"(rC[22]), "+f"(rC[23]), "+f"(rC[24]),
        "+f"(rC[25]), "+f"(rC[26]), "+f"(rC[27]), "+f"(rC[28]), "+f"(rC[29]),
        "+f"(rC[30]), "+f"(rC[31])
