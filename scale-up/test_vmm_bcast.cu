...
/*
nvcc test_vmm_bcast.cu -o test_vmm_bcast \
...
/// Forward memory handle via file handles and unix sockets
/// If IMEX (internode memory exchange) manager provided, 
/// you may also use CU_MEM_HANDLE_TYPE_FABRIC, which is easier to implement.
auto fwd_mem_handle(
...
int main(int argc, char **argv) {
    ...
    if(rank == 0) {
        ...
        bcast_krnl<<< (nelem + 255) / 256, 256>>>(reinterpret_cast<float *>(d_buf_mc), test_buf_d_root, nelem);
        CUDART_CHECK(cudaDeviceSynchronize());
        CUDART_CHECK(cudaFree(test_buf_d_root));
    }
    MPI_CHECK(MPI_Barrier(MPI_COMM_WORLD));
    float *bcast_mc = new float[nelem];
    CUDA_CHECK(cuMemcpyDtoH(bcast_mc, d_buf_loc, aligned_size));
